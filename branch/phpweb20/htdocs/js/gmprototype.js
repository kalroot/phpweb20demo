...
BlogLocationManager.prototype =
{
	...
	loadMap : function()
	{
		...
		this.addMarkerToMap(0, 28.139878, 120.289431, 'Qing Tian');
		this.addMarkerToMap(1, 30.139878, 122.289431, 'Second Marker');
	...
	zoomAndCenterMap : function()
	{
		...
		if (bounds.isEmpty())
		{
			//alert('bounds is empty!');
			this.map.setCenter(new google.maps.LatLng(0, 0), 1, G_HYBRID_MAP);
		}
		else
		{
			//alert('bounds is not empty');
			//alert(this.map.getBoundsZoomLevel(bounds));
...
/*
	...
	dragComplete : function(marker)
	{
		...
		var location_ids = this.markers.keys();
		var i = location_ids.indexOf(marker.location_id);
		...
		this.addMarkerToMap(marker.location_id, point.lat(), point.lng(), marker.title);
		...
		//alert('this.marker[1] = ' + this.markers[1].title);
	},
...
/*
		...
		this.removeMarkerFromMap(location_id);
	},
