...
class Model_DatabaseObject_BlogPost extends Model_DatabaseObject
{
	public $profile = null;
	public $images = array();
	...
	public function __construct($db)
	{
		...
		$this->profile = new Model_Profile_BlogPost($db);
	}
	...
	protected function postLoad()
	{
		...
		$this->images = Model_DatabaseObject_BlogPostImage::GetImages($this->getDb(), $options);
		$this->locations = Model_DatabaseObject_BlogPostLocation::GetLocations($this->getDb(), $options);
	}
	...
	public static function GetPosts($db, $options = array())
	{
		...
		$profiles = Model_Profile::BuildMultiple($db, 
						'Model_Profile_BlogPost', array('post_id' => $post_ids));
		
		foreach ($posts as $post_id => $post)
		{
			if (array_key_exists($post_id, $profiles) &&
				$profiles[$post_id] instanceof Model_Profile_BlogPost)
		...
		$images = Model_DatabaseObject_BlogPostImage::GetImages($db, $options);
		foreach ($images as $image)
			$posts[$image->post_id]->images[$image->getId()] = $image;

		$locations = Model_DatabaseObject_BlogPostLocation::GetLocations($db, $options);
